from . import procces_data, check_prep_files
...
def _tfInference(text):
    ...
    ginfo = GINFO
    # print(ginfo)
    # feel free to adapt the conv enty and the first name (info_1) and last name (info_2) you
    # want it to return
    # with making the datapoints I do not use the dict which start with _ for making identifing
    # entities
    ...
    #     print(np.round(y_,1))
    # print(g_.shape)
    return ginfo['procces'].get_info(y = y_, x = x_, seq_len = seq_len, intent = g_,
                                     sen_dict = sens_dict, plot = False)
#     y_output_model = y_
