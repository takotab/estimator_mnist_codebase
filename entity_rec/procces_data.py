...
class prep_data:
    def __init__(self, dialog_class, data_filename = "None", 
                 ...
                 chance_on_name = 0.90, threshold = 0.8, add_eos = False):
        if "*" in data_filename:
            data_filename = self.find_latest(data_filename)
        ...
        self.goal_legend = ["intent_unkown", "intent_weather", "intent_sql_lookup",
                            "intent_sql_change", "intent_sql_count", "intent_waterstand_doorgeven","intent_stop",
                            "intent_vacation_days", "intent_waterstand_vergelijken","intent_factuur","intent_address_change"]
        self.goal_legend += ["intent_dwg_" + str(i + 1) for i in range(30)]
        self.goal_dict = {abc: i for i, abc in enumerate(self.goal_legend)}
        self.goal_dict_rev = {i: abc for i, abc in enumerate(self.goal_legend)}
    ...
    def extra_info_def(self):
        ...
        self.list_w_f_names = []
        list_w_names = os.path.join(config.PREPERATION_DIR,"list_w_first_names.txt")
        with open(list_w_names, 'r') as f:
            for name in f:
        ...
        self.list_w_l_names = []
        list_w_last_names = os.path.join(config.PREPERATION_DIR,"list_w_last_names.txt")
        with open(list_w_last_names, 'r') as f:
            for name in f:
    ...
    def make_datapoint(self, df_dict, direct = True):
        self.dialog_.num_unknown = 0
        self.dialog_.missing_words = []
        ...
        for j, key in enumerate(list_w_keys):
            ...
            if not df_dict[key] == "0" and not df_dict[key] == 0:
                ...
                # array[start_name:finish_name, 291] = (j+1)*1000
                if start_name == 0 and finish_name == 0:
                    print("conv_str", df_dict["_conv"])
                    print(key, "_str", df_dict[key])
                # print(_word, "will become ", key)
                # +1 because 0 is nothing
                y = self.make_y(y, start_name, finish_name, j+1)

        if '_intent' in df_dict and not df_dict['_intent'] == 0 and not df_dict['_intent'] == '0':
            df_dict['_intent'] = self.goal_dict[df_dict['_intent']]
            
        else:
            df_dict['_intent'] = 0
            
        # if df_dict['_intent'] > 5:
        #     print(df_dict['_intent'], self.dialog_.num_unknown)
        #     self.missing_words += self.dialog_.missing_words
        ...
        if direct:
            ...
            intent = np.array(df_dict['_intent'])[np.newaxis]
            return intent, y, np.array([int(sequence_length_)]), array

    def get_batch(self,seed = None, get_a_conv = False):
        self.dialog_.num_unknown = 0
        # print("current setting is cheating!!!", 291)
        if seed is None:
            self.seed += 1
        ...
        # print("self.dialog_.num_unknown", self.dialog_.num_unknown)
        if get_a_conv:
...
#######################################################################################################################
    ...
    def get_info(self, do_no_use = None, y = None, x = None
        , seq_len= None, intent = [], sen_dict = None, plot = False):
        assert do_no_use == None
        ...
        # print(y.shape, seq_len.shape)
        y = y[:seq_len, :]
        x = x[:seq_len, :]
        # plus one because you have all the entitys plus the _intent
        y_ = np.zeros((y.shape[0], len(self.current_info)+1))
        # print(y_.shape,y.shape) #  (7, 8) (7, 20)
        y_[:y.shape[0], :y.shape[1]] = y
        y = y_
        dict_w_names = {}
        for j in range(len(self.current_info)):
            confidence = np.round(np.max(y[:,j+1]),3)
            if confidence > self.threshold:
                mask = np.where(y[:,j+1] > self.threshold)
                if sen_dict == None:
                    x_info = x[mask,:]
    ...
    def find_latest(self, data_filename):
        import glob
        import os
...
if __name__ == "__main__":
    ...
    sequence_length_, array, sentence_dict = prep.make_datapoint({"_conv":"hello hello Alphen aan den Rijn","nlp_loc":"Alphen aan den Rijn"})
